<template>
  <div class="page">
          ...
          <el-form ref="form" :model="postForm" label-width="80px" class="elForm">
            <el-col :span="5">
              <el-form-item label="角色名称:">
                <el-input v-model="postForm.name" size="small" placeholder="输入角色名称" />
              </el-form-item>
            </el-col>
          ...
          <el-table-column prop="done" label="操作" align="center">
            <template slot-scope="scope">
              <el-button-group class="btnGroup">
                <el-button size="mini" type="success" @click="editForm">编辑</el-button>
                <el-button size="mini" type="warning" @click="operaForm">操作权限</el-button>
                <el-button size="mini" type="primary" @click="dataForm">数据权限</el-button>
    ...
    <el-dialog title="新增/编辑 角色" :visible.sync="dialogSettingFormVisible" class="newDialog" @close="cancelSettingForm">
      <el-form ref="settingForm" :model="settingForm" :rules="settingForm.settingFormRules">
        <el-form-item label="角色名称" label-width="80px" prop="roleName">
          <el-input v-model="settingForm.roleName" name="roleName" autocomplete="off" />
        </el-form-item>
        <el-form-item label="启用状态" label-width="80px">
          <el-switch v-model="settingForm.status" />
        </el-form-item>
    ...
    <el-dialog title="在内员工" :visible.sync="innnerEmployee">
      <el-form>
        <el-form-item label="角色名称" label-width="80px" prop="roleName">
          <el-input v-model="innnerEmployeeTable.roleName" name="roleName" autocomplete="off" disabled />
        </el-form-item>
        <template>
          <el-table
      ...
      <div slot="footer">
        <el-button type="danger" @click="innnerEmployee = false">关闭</el-button>
      </div>
    </el-dialog>
    <!-- 操作权限 -->
    <el-dialog title="设置操作权限" :visible.sync="operaRule">
      <el-form>
        <el-tree :data="operaTree" :props="defaultProps" check-strictly="true" show-checkbox @node-click="handleTreeClick" />
      </el-form>
      <div slot="footer">
        <el-button type="success" @click="operaRule = false">提 交</el-button>
        <el-button @click="operaRule = false">取 消</el-button>
      </div>
    </el-dialog>
    <!-- 设置数据权限 -->
    <el-dialog title="设置数据权限" :visible.sync="dataRule">
      <el-table ref="multipleTable" :data="dataRuleTable" border style="width: 100%">
        <el-table-column prop="tap" label="选项" />
        <el-table-column prop="demo" label="示例" />
        <el-table-column label="正常">
          <template scope="scope">
            <el-radio v-model="scope.row.on" label="正常" @change="getData" />
          </template>
        </el-table-column>
        <el-table-column label="脱敏">
          <template scope="scope">
            <el-radio v-model="scope.row.on" label="脱敏" @change="getData" />
          </template>
        </el-table-column>
      </el-table>
      <div slot="footer">
        <el-button type="success" @click="dataRule = false">提 交</el-button>
