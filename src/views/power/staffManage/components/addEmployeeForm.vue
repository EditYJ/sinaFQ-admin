    ...
    <el-dialog title="新增/编辑 员工" :visible.sync="editDiglog" class="addDialog" top="2.5vh" center="true" @close="resetForm('employeeForm')">
      <el-form ref="employeeForm" :model="employeeForm" :rules="employeeForm.rules" label-width="80px" label-position="left">
        <el-form-item label="账号" prop="account">
          <el-input v-model="employeeForm.account" placeholder="4~20长度的字母数字组合，须字母开头" />
        </el-form-item>
        <el-form-item label="姓名" prop="name">
          <el-input v-model="employeeForm.name" placeholder="请输入姓名" />
        </el-form-item>
        <el-form-item label="手机号" prop="tel">
          <el-input v-model="employeeForm.tel" placeholder="请输入手机号" />
        </el-form-item>
        <el-form-item label="微信号" prop="wechat">
          <el-input v-model="employeeForm.wechat" placeholder="请输入微信号" />
        </el-form-item>
        <el-form-item label="邮箱地址" prop="email">
          <el-input v-model="employeeForm.email" placeholder="请输入邮箱地址" />
        </el-form-item>
        <el-form-item label="员工状态">
          <el-switch v-model="employeeForm.employeeStatus" />
        </el-form-item>
        <el-form-item label="员工描述" prop="desc">
          <el-input v-model="employeeForm.desc" type="textarea" placeholder="请输入员工描述（选填）" />
        </el-form-item>
        <el-form-item label="所属角色">
          <el-checkbox-group v-model="employeeForm.innerRole">
            <el-checkbox label="超级管理员" name="type" />
            <el-checkbox label="商品售后" name="type" />
            <el-checkbox label="用户分析" name="type" />
            <el-checkbox label="商城运营" name="type" />
            <el-checkbox label="催收员" name="type" />
            <el-checkbox label="财务" name="type" />
          </el-checkbox-group>
        </el-form-item>
        <el-form-item>
